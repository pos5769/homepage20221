		...
		<c:choose>
			...
			<c:when test="${step eq '5'}">
				<li>
					...
					<c:set var="test" value="0"/>
					<c:choose>
						<c:when test="${test eq '0'}">1입니다.</c:when> <%-- value가 0이면 1입니다.가 출력 --%>
						<c:otherwise>조건에 해당되지 않음</c:otherwise> <%-- value가 0이 아니면 조건에 해당되지 않음이 출력 --%>
					</c:choose>
				</li>
			</c:when>
			...
			<c:when test="${step eq '8'}">
				<li>
					...
					<a href="${link1}" target="_blank" title="새 창 열기(네이버 페이지로 이동하기)">링크 1 번</a><br/> <%-- target="_blank" => 새 창 열기 --%>
			...
			<c:when test="${step eq '18'}">
				<li>
					<h3>fn:replace() : 모든 문자열을 다른 문자욜 시퀀스로 바꿈</h3>
